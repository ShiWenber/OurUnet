...
def augment_data(images, masks, save_path, augment=True):
    ...
    for image, mask in tqdm(zip(images, masks), total=len(images)):
        ...
        if augment == True:
            ...
            # MedianBlur supports only odd blur limits.
            aug = MedianBlur(p=1, blur_limit=7)
            augmented = aug(image=x, mask=y)
            x21 = augmented['image']
            y21 = augmented['mask']

            # aug = GaussianBlur(p=1, blur_limit=10)
            # GaussianBlur supports only odd blur limits. 
            augmented = aug(image=x, mask=y)
            x22 = augmented['image']
            y22 = augmented['mask']
...
def temp_xirou():
    np.random.seed(42)
    path = "./xirou/"
    train_i = glob(os.path.join(path, "train/image/*"))
    train_l = glob(os.path.join(path, "train/label/*"))
    test_i = glob(os.path.join(path, "test/image/*"))
    test_l = glob(os.path.join(path, "test/label/*"))
    ...
    augment_data(train_i, train_l, "new_data/train/", augment=True)
    augment_data(test_i, test_l, "new_data/test/", augment=False)
